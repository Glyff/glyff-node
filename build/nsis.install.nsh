Name "glyff ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install glyff binary
Section "Glyff" GLYFF_IDX
  SetOutPath $INSTDIR
  file {{.Glyff}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\glyff.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\glyff.exe" "attach" "" ""
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Glyff incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Glyff outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Glyff UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Glyff incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\glyff.exe" "" "" "Ethereum" 30303 "" "" ""
  SimpleFC::AdvAddRule "Glyff outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\glyff.exe" "" "" "Ethereum" "" 30303 "" ""
  SimpleFC::AdvAddRule "Glyff UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\glyff.exe" "" "" "Ethereum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\glyff.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\glyff.ipc"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GLYFF_IDX}
    SectionGetSize ${GLYFF_IDX} $0
