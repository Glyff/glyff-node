...
#
# The cluster contains a bootnode, a glyff node and multiple swarm nodes, with
...
# static glyff configuration
GLYFF_IP="192.168.33.3"
GLYFF_RPC_PORT="8545"
GLYFF_RPC_URL="http://${GLYFF_IP}:${GLYFF_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the glyff and
# swarm nodes
start_bootnode() {
...
# start_glyff_node starts a glyff node with --datadir pointing at <base-dir>/glyff
# and a single, unlocked account with password "glyff"
start_glyff_node() {
  local dir="${base_dir}/glyff"
  mkdir -p "${dir}"

  local password="glyff"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating glyff account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get glyff account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GLYFF_IP}"
    --rpcport   "${GLYFF_RPC_PORT}"
    --verbosity "6"
  )

  start_node "glyff" "${GLYFF_IP}" "$(which glyff)" ${args[@]}
}
...
create_account() {
  ...
  glyff --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
